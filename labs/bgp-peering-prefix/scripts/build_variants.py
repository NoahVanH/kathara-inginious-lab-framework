...
def ip_from_ref(state: dict, ref: dict) -> str:
    return ip_only(cidr_from_ref(state, ref))


def find_bgp_neighbor_by_ip(state: dict, node: str, neighbor_ip: str):
    bgp = state[node].get("bgp")
    if not bgp:
        return None, None

    for family in ("ibgp_neighbors", "ebgp_neighbors"):
        for neigh in bgp.get(family, []):
            if neigh["ip"] == neighbor_ip:
                return neigh, family

    return None, None


def find_bgp_neighbor_by_node(state: dict, src_node: str, dst_node: str):
    bgp = state[src_node].get("bgp")
    if not bgp:
        return None, None
    ...
    for family in ("ibgp_neighbors", "ebgp_neighbors"):
        for neigh in bgp.get(family, []):
            if neigh.get("neighbor_node") == dst_node:
                return neigh, family
...
def apply_error(student_state: dict, error_name: str, errors_def: dict):
    ...
    # nouveaux kinds BGP fins
    elif kind == "bgp_remove_activate":
        node = err["node"]
        neighbor_ip = ip_from_ref(student_state, err["neighbor_ip_from"])
        neigh, family = find_bgp_neighbor_by_ip(student_state, node, neighbor_ip)
        if neigh is not None:
            neigh["activate"] = False
    ...
    elif kind == "bgp_remove_next_hop_self":
        node = err["node"]
        neighbor_ip = ip_from_ref(student_state, err["neighbor_ip_from"])
        neigh, family = find_bgp_neighbor_by_ip(student_state, node, neighbor_ip)
        if neigh is not None:
    ...
    elif kind == "bgp_wrong_neighbor_ip":
        node = err["node"]
        neighbor_ip = ip_from_ref(student_state, err["neighbor_ip_from"])
        neigh, family = find_bgp_neighbor_by_ip(student_state, node, neighbor_ip)
        if neigh is not None:
            neigh["ip"] = err["wrong_neighbor_ip"]

    elif kind == "bgp_remove_update_source":
        node = err["node"]
        neighbor_ip = ip_from_ref(student_state, err["neighbor_ip_from"])
        neigh, family = find_bgp_neighbor_by_ip(student_state, node, neighbor_ip)
        if neigh is not None:
    ...
    elif kind == "bgp_wrong_remote_as":
        node = err["node"]
        neighbor_ip = ip_from_ref(student_state, err["neighbor_ip_from"])
        neigh, family = find_bgp_neighbor_by_ip(student_state, node, neighbor_ip)
        
        #print(f"DEBUG found_neigh={neigh} {neigh["remote_as"]} {err["wrong_as"]} ")
        if neigh is not None:
            neigh["remote_as"] = err["wrong_as"]
