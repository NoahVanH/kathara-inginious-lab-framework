...
def enrich_with_protocols(expected: dict, protocols: dict,schemes: dict, scheme_name: str):
    """
    ...
    for node_name, proto in protocols.items():
        ...
        if dns:
            ...
            # bind ip must be valide and on one of the interfaces
            for iface_name, iface_data in expected[node_name]["interfaces"].items():
                #print(f"Checking DNS bind_ip for {node_name} iface {iface_name} with cidr {iface_data['cidr']}")
                if(iface_name == "lo"):
                    #TODO: for now only the loopback
                    dns_state["bind_ip"] = ip_only(iface_data["cidr"])
                    dns_state["server_ip"] = ip_only(iface_data["cidr"])
                    record_ip = ip_only(iface_data["cidr"])
                    break
            
            for record in dns.get("records", []):
                #random only last bit of record_ip
                record_ip = dns_state["server_ip"][:-1] + randint(1, 254).__str__()
                #print(f"Processing DNS record {record.get('name')} for node {node_name}")
                dns_state["records"].append({
                    "name": record["name"],
                    "ip": record_ip,
                    "type": record["type"],
                    
                })
            dns_state["nameserver"] = dns.get("nameserver")
            dns_state["type"] = dns.get("type")
            #print(f"Final DNS state for {node_name}: {dns_state}")
            expected[node_name]["dns"] = dns_state
        # enrichissement BGP
        bgp = proto.get("bgp")
        if bgp:
            bgp_state = {
                ...
                "ebgp_neighbors": [],
                "ibgp_neighbors": [],
                "anycast_prefixes": None,
            }

            anycast_net_name = bgp.get("anycast_prefixes")
            anycast_cidr = None

            if anycast_net_name:
                current_scheme = schemes[scheme_name]
                anycast_cidr = current_scheme["subnets"].get(anycast_net_name)
                #print(f"Anycast CIDR for {node_name}: {anycast_cidr}")

            for ifname in bgp.get("network_interfaces", []):
                if expected[node_name].get("dns") and anycast_cidr:
                    bgp_state["networks"].append(anycast_cidr)
                    bgp_state["anycast_prefixes"] = anycast_cidr
                else:
                    bgp_state["networks"].append(
                        expected[node_name]["interfaces"][ifname]["cidr"]
                    )
...
def enrich_with_isis_sr(expected: dict, protocols: dict):
    """
    Copy IS-IS / SR / TI-LFA / MPLS settings from protocols.yaml into the
    resolved state so templates can read them via student[node].{isis,sr,...}.
    """
    for node_name, proto in protocols.items():
        if node_name not in expected:
            continue
        # Only relevant for the isis/sr template family.
        if proto.get("template") != "router_isis_sr":
            continue
 
        # Deep-ish copies so apply_error() can mutate them on the student
        # state without affecting the expected one.
        from copy import deepcopy
        expected[node_name]["isis"]   = deepcopy(proto.get("isis",   {}))
        expected[node_name]["sr"]     = deepcopy(proto.get("sr",     {}))
        expected[node_name]["ti_lfa"] = deepcopy(proto.get("ti_lfa", {}))
        expected[node_name]["mpls"]   = deepcopy(proto.get("mpls",   {"enabled": True}))
...
def cidr_from_ref(state: dict, ref: dict) -> str:
    return state[ref["node"]]["interfaces"][ref["interface"]]["cidr"]
...
def apply_error(student_state: dict, error_name: str, errors_def: dict):
    ...
    elif kind == "bgp_remove_network":
        node = err["node"]
        bgp = student_state[node].get("bgp")
        if not bgp:
            return
        network_cidr = cidr_from_ref(student_state, err["network_from"])
        bgp["networks"] = [net for net in bgp.get("networks", []) if net != network_cidr]

    elif kind == "bgp_replace_network_with_ref":
        node = err["node"]
        bgp = student_state[node].get("bgp")
        if not bgp:
            return
        # réseau à remplacer (dans bgp["networks"] du nœud)
        target_network = cidr_from_ref(student_state, err["network_target"])
        # nouvelle IP prise depuis une référence externe
        new_ip = ip_only(cidr_from_ref(student_state, err["network_from"]))
        new_network = f"{new_ip}/{err['new_prefixlen']}"
        print(f"Replacing BGP network {target_network} with {new_network} on node {node}")
        bgp["networks"] = [
            new_network if net == target_network else net
            for net in bgp.get("networks", [])
        ]
    elif kind == "bgp_replace_network_prefixlen":
        node = err["node"]
        bgp = student_state[node].get("bgp")
        if not bgp:
            return
        old_network = cidr_from_ref(student_state, err["network_from"])
        new_network = replace_prefixlen(old_network, err["new_prefixlen"])
        print(f"Replacing BGP network {old_network} with {new_network} on node {node}")
        bgp["networks"] = [
            new_network if net == old_network else net
            for net in bgp.get("networks", [])
        ]
    # DNS RELATED ERRORS
    elif kind == "dns_wrong_nameserver":
        node = err["node"]
        dns = student_state[node].get("dns")
        if not dns:
            return
        dns["nameserver"] = err["new_nameserver"]

    elif kind == "dns_wrong_domain":
        node = err["node"]
        dns = student_state[node].get("dns")
        if not dns:
            return
        dns["domain"] = err["new_domain"]

    elif kind == "dns_wrong_bind_ip":
        node = err["node"]
        dns = student_state[node].get("dns")
        if not dns:
            return
        dns["bind_ip"] = ip_only(
            cidr_from_ref(student_state, err["bind_ip_from"])
        )
        print(f"DEBUG dns after error: {student_state[node]['dns']}")
...
def render_generic_frr_configs(
    env: Environment,
    variant_dir: Path,
    expected: dict,
    student: dict,
    ...
    """
    Rend les fichiers frr.conf pour tous les routeurs.
    """

    template_map = {
        "router_ospf6_bgp": "router_ospf6_bgp.conf.j2",
        "router_bgp_only": "router_bgp_only.conf.j2",
        "router_isis_sr": "router_isis_sr.conf.j2",
    }
    ...
    for node_name, node in student.items():
        ...
        if not template_name:
            raise ValueError(
                f"Template inconnu pour {node_name}: {template_key}"
            )
...
def render_dns_configs(
    ...
    """
    Rend tous les fichiers DNS nécessaires.
    """

    dns_templates = [
        {
            "template": "dns/named.conf.local.j2",
            "output": "etc/bind/named.conf.local",
        },
        {
            "template": "dns/named.conf.options.j2",
            "output": "etc/bind/named.conf.options",
        },
        {
            "template": "dns/db.domain.j2",
            "output": "--- dynamic output path based on domain ---", 
        }
      
    ]

    for node_name, node in student.items():
        print(f"Processing DNS templates for {node_name}")

        dns = expected[node_name].get("dns")
        ...
        if not dns:
            continue
        #print(f"Node {node_name} has DNS configuration: {dns}")
        for dns_file in dns_templates:
            
            if dns_file["template"] == "dns/db.domain.j2":
                output_relative = f"etc/bind/zones/db.{dns['domain']}"
            else:
                 output_relative = dns_file["output"]
            output_path = variant_dir / node_name / output_relative
            ...
            render_template_to_file(
                env=env,
                template_name=dns_file["template"],
                output_path=output_path,
                context=context,
            )
...
def render_dynamic_files(env, variant_dir, expected, student, variant_id):
    """
    Rend tous les .j2 sauf les templates génériques FRR/DNS/startup,
    puis rend les fichiers génériques explicitement.
    """
    ...
    for template_path in TEMPLATE_DYNAMIC_DIR.rglob("*.j2"):
        ...
        if relative_template_path.parts[0] == "dns":
                continue
...
def build_one_variant(env: Environment, variant: dict, base_topology: dict, protocols: dict, schemes: dict, errors_def: dict):
    ...
    enrich_with_protocols(expected, protocols, schemes, scheme_name)
    enrich_with_isis_sr(expected, protocols) # maybe merge them
