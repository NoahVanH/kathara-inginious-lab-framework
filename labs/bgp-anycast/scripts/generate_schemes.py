...
def rand_anycast_net(used: set) -> str:
    """Generate a /64 anycast subnet (2001:db8:XX::/64)."""
    while True:
        v = random.randint(0x10, 0xFF)
        h = format(v, 'x')
        if h not in used:
            used.add(h)
            return f"2001:db8:{h}::/64"
# ---- IPv6 ----
...
def ipv6_schemes(domains: list, nodes: list) -> dict:
    ...
    for name, use_prefix, lo_base, subnet_fn in variants:
        ...
        # =========================
        # ANYCAST (FIXED IP, NOT NET)
        # =========================
        anycast_used = set()

        # choose a prefix for anycast inside same addressing plan
        anycast_prefix = rand_hex2(set())

        # derive stable "service host id"
        anycast_host = "53"  # DNS-like, can be randomized if needed

        anycast_ip = f'2001:db8:1::{anycast_host}/128'

        subnets['anycast_net'] = anycast_ip
        print(f"Generated anycast IP for {name}: {anycast_ip}")

        schemes[f'scheme_{name}'] = {
            'subnets': subnets
        }
