"""
Build and generate parametrized Kathara/INGInious lab variants.

This script automates the generation of multiple network lab variants from a
single logical topology model. It combines static files, Jinja2 templates,
addressing schemes, protocol definitions, and injected configuration errors
to produce complete deployable Kathara lab archives.

Main features
-------------
- Load topology, protocol, addressing, error, and variant definitions from YAML.
- Resolve logical network models into concrete interface and routing states.
- Generate FRRouting (FRR) and startup configurations using Jinja2 templates.
- Inject controlled Layer 3, OSPF, iBGP, and eBGP misconfigurations.
- Build isolated lab variant directories.
- Produce ZIP archives ready for INGInious/Kathara deployment.
- Export a manifest describing all generated variants.

Project structure
-----------------
template/static/
    Static files copied directly into each variant.

template/dynamic/
    Jinja2 templates rendered dynamically for each variant.
...
2. Load YAML models.
3. Resolve the expected network state.
4. Enrich the state with routing protocol information.
5. Clone the expected state into a student state.
6. Apply variant-specific configuration errors.
7. Render all dynamic templates.
8. Copy static resources.
9. Generate ZIP archives and manifest metadata.

Generated outputs
-----------------
For each variant:
- Fully rendered Kathara lab directory.
- Router startup scripts.
- FRR configuration files.
- Metadata description.
- Deployable ZIP archive.

The final manifest.json summarizes all generated variants and their associated
configuration schemes and injected errors.


Docstring generated by ChatGPT, May 2026
"""
...
def resolve_expected_state(base_topology: dict, scheme: dict) -> dict:
    """
    Transforms the logical topology model into the expected concrete network state.

    Supports:
    - Standard interfaces using host_offset addressing
    - /128 loopback interfaces without host_offset
    """
...
def enrich_with_protocols(expected: dict, protocols: dict):
    """
    Adds template, OSPFv3, and BGP protocol configuration data to the expected
    network state from protocols.yaml.
    """
    for node_name, proto in protocols.items():
        ...
        #  OSPF enrichissement interfaces
...
def apply_error(student_state: dict, error_name: str, errors_def: dict):
    ...
    # For more errors add elif blocks here
...
def render_generic_frr_configs(env: Environment, variant_dir: Path, expected: dict, student: dict):
    """
    Render FRR configuration files from the generic routing templates:
...
def render_dynamic_files(env, variant_dir, expected, student, variant_id):
    """
    Render all Jinja2 templates except the generic FRR templates,
    then generate FRR configurations using the dedicated generic templates.
    """
