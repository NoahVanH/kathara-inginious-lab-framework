...
def apply_error(student_state: dict, error_name: str, errors_def: dict):
    ...
    # OSPF ERRORS
    # =========================
    elif kind == "wrong_ospf_cost": # TODO : BUGGY NOT APPLICATE
        node = err["node"]
        iface = err["interface"]
        if "ospf6" in student_state[node]["interfaces"][iface]:
            student_state[node]["interfaces"][iface]["ospf6"]["cost"] = err["new_cost"]
            
    elif kind == "eth_passive":
        #node.ospf6.interfaces[ifname].passive
        node = err["node"]
        iface = err["interface"]
     
        #print(f"Applying eth_passive error on OSPF6 passive for interface {iface} of node {node}")
        student_state[node]["interfaces"][iface]["ospf6"]["passive"] = True
        #print(f"Resulting OSPF6 config for {node} {iface}: {student_state[node]['interfaces'][iface]['ospf6']}")
    
    elif kind == "wrong_router_id":
        node = err["node"]  
        new_router_id = err["new_router_id"]
        student_state[node]["ospf6"]["router_id"] = new_router_id
        
    elif kind == "wrong_mtu":
        node = err["node"]
        iface = err["interface"]
        new_mtu = err["new_mtu"]
        student_state[node]["interfaces"][iface]["ospf6"]["ifmtu"] = new_mtu
...
def render_generic_frr_configs(env: Environment, variant_dir: Path, expected: dict, student: dict):
    ...
    - router_ospf6.conf.j2
    - router_bgp_only.conf.j2
    """
    template_map = {
        "router_ospf6": "router_ospf6.conf.j2",
