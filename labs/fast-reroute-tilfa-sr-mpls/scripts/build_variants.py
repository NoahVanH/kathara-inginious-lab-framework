...
def apply_error(student_state: dict, error_name: str, errors_def: dict):
    ...
    # BGP ROLES ERRORS
        ...
        for dst in err["neighbors"]:
            neigh = find_ebgp_neighbor(student_state, node, dst)
            if neigh:
                neigh["role_override"] = wrong_role
                
    elif kind == "bgp_wrong_role_pair":
        a = err["a_node"]
        b = err["b_node"]

        a_neigh = find_ebgp_neighbor(student_state, a, b)
        b_neigh = find_ebgp_neighbor(student_state, b, a)

        if a_neigh:
            a_neigh["role_override"] = err["a_role"]

        if b_neigh:
            b_neigh["role_override"] = err["b_role"]
            
            
    # Fast Reroute / SR ERRORS
    elif kind == "sr_disabled":
        for node in err["nodes"]:
            sr = student_state[node].get("sr")
            if sr is not None:
                sr["enabled"] = False
    
    elif kind == "sr_wrong_index":
        wrong = err["wrong_index"]
        for node in err["nodes"]:
            sr = student_state[node].get("sr")
            if sr is not None:
                sr["index"] = wrong
    
    elif kind == "tilfa_interface_disabled":
        node  = err["node"]
        iface = err["interface"]
        isis  = student_state[node].get("isis") or {}
        if_map = isis.get("interfaces") or {}
        if iface in if_map:
            if_map[iface]["ti_lfa"] = False
    
    elif kind == "tilfa_no_node_protection":
        for node in err["nodes"]:
            ti_lfa = student_state[node].get("ti_lfa")
            if ti_lfa is not None:
                ti_lfa["node_protection"] = False
                ti_lfa["link_fallback"]   = False   # link-fallback requires node-protection
